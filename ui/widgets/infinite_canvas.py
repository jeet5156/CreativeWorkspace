...
class InfiniteCanvas(QGraphicsView):
    ...
    def set_context(self, context):
        ...
        if not hasattr(self, "node_context") or not self.node_context:
            self.node_context = NodeContext()
        if context:
            self.node_context.app_context = context
        self.node_context.project = getattr(self, "project", None)
        self.node_context.board_id = getattr(self, "_board_id", None)
        for node in self._items_map.values():
            node.project = getattr(self, "project", None)
            node._board_id = getattr(self, "_board_id", None)
    ...
    def contextMenuEvent(self, event):
        ...
        if isinstance(item, NodeItem):
            ...
            if len(selected_nodes) > 1:
                ...
                menu.addAction(create_frame_act)
                menu.addSeparator()
            # Promotion actions (Move to Project / Copy to Project)
            menu.addSeparator()
            move_proj_act = QAction("📁  Move to Project...", self)
            move_proj_act.triggered.connect(lambda checked=False, n=item: self._prompt_promote_nodes(n, action="move"))
            menu.addAction(move_proj_act)
            copy_proj_act = QAction("📋  Copy to Project...", self)
            copy_proj_act.triggered.connect(lambda checked=False, n=item: self._prompt_promote_nodes(n, action="copy"))
            menu.addAction(copy_proj_act)
            menu.addSeparator()
    ...
    def _create_node_from_def(self, defn: NodeDefinition, scene_pos: QPointF):
        ...
        if node:
            node.setSelected(True)
    def _prompt_promote_nodes(self, item=None, items=None, action: str = "move"):
        """Prompt user with NodePromotionDialog to Move or Copy a set of nodes to a target Project/Workbench board."""
        sel_nodes = self.selected_nodes()
        if items:
            target_items = list(items)
        elif item:
            target_items = sel_nodes if (hasattr(item, "isSelected") and item.isSelected() and len(sel_nodes) > 1) else [item]
        else:
            target_items = sel_nodes
        if not target_items:
            return
        from ui.dialogs.node_promotion_dialog import NodePromotionDialog
        from PySide6.QtWidgets import QMessageBox, QDialog
        app_ctx = getattr(self, "_context", None)
        if not app_ctx and hasattr(self, "node_context") and self.node_context:
            app_ctx = getattr(self.node_context, "app_context", None)
        if not app_ctx and hasattr(self, "parent") and self.parent():
            app_ctx = getattr(self.parent(), "_context", None)
        # Find parent LabPanel if available
        lab_panel = None
        curr = self.parent() if callable(getattr(self, "parent", None)) else None
        while curr:
            if hasattr(curr, "_current_project") and hasattr(curr, "_current_board_id"):
                lab_panel = curr
                break
            curr = curr.parent() if callable(getattr(curr, "parent", None)) else None
        proj_svc = getattr(app_ctx, "project_service", None) if app_ctx else None
        lab_svc = getattr(app_ctx, "lab_service", None) if app_ctx else None
        if not lab_svc:
            from services.lab_service import LabService
            lab_svc = LabService(project_service=proj_svc)
        first_item = target_items[0] if target_items else None
        source_project = getattr(self, "project", None) or getattr(first_item, "project", None) or (getattr(lab_panel, "_current_project", None) if lab_panel else None) or (getattr(app_ctx, "current_project", None) if app_ctx else None)
        source_board_id = getattr(self, "_board_id", None) or getattr(first_item, "_board_id", None) or (getattr(lab_panel, "_current_board_id", None) if lab_panel else None) or lab_svc.get_active_board_id(source_project) or "Main"
        dialog = NodePromotionDialog(self, action, source_project, source_board_id, proj_svc, lab_svc, target_nodes=target_items)
        if dialog.exec_() != QDialog.Accepted:
            return
        p_target, b_id_target = dialog.get_selected_destination()
        if not b_id_target:
            return
        target_node_ids = [n.id for n in target_items if hasattr(n, "id")]
        ...
        try:
            if action.lower() == "move":
                res_items = lab_svc.move_nodes(source_project, source_board_id, p_target, b_id_target, target_node_ids)
                if res_items:
                    moved_ids = {it["id"] for it in res_items if isinstance(it, dict) and "id" in it}
                    scene_obj = self.scene() if callable(getattr(self, "scene", None)) else getattr(self, "_scene", None)
                    for nid in list(moved_ids):
                        if nid in self._items_map:
                            node_item = self._items_map.pop(nid)
                            if scene_obj and node_item:
                                scene_obj.removeItem(node_item)
                    for conn_id, conn in list(self._connector_map.items()):
                        if conn.source_id in moved_ids or conn.target_id in moved_ids:
                            if scene_obj and conn:
                                scene_obj.removeItem(conn)
