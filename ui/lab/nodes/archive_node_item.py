...
class ArchiveNodeItem(NodeItem):
    """Archive Reference spatial node item for the Lab canvas.

    Renders lightweight document cards displaying archive references (.zip, .rar, .7z, etc.),
    format indicator, cached file size, missing file detection, and locate/relink capabilities.
    Zero archive content extraction or indexing is performed.
    """
    ...
    def __init__(self, definition: NodeDefinition, parent=None):
        ...
        self.state = ArchiveNodeState.EMPTY
    ...
    def from_dict(self, data: dict):
        ...
        if "display_mode" not in self.payload:
            self.payload["display_mode"] = "icon"
        if self.payload.get("archive_path") or self.payload.get("absolute_path") or self.payload.get("image_path"):
            self.validate_reference(force=True)
        else:
            self.state = ArchiveNodeState.EMPTY
    ...
    def set_archive(self, archive_path: str, project_location: str = None):
        """Programmatic API to set or replace Archive reference."""
        self._last_validated_time = 0.0
        ...
        abs_path = Path(archive_path).resolve()
        ...
        if not abs_path.exists():
            self.payload["archive_path"] = str(archive_path)
            self.payload["image_path"] = str(archive_path)
            self.payload["file_size_str"] = ""
            self.state = ArchiveNodeState.MISSING
            self._emit_modified()
            self.update()
            return
        ...
        self.payload["archive_path"] = rel_path
        self.payload["image_path"] = rel_path
        self.state = ArchiveNodeState.READY
        self._emit_modified()
        self.update()

    set_file = set_archive
    set_asset = set_archive
    set_pdf = set_archive
    set_image = set_archive

    def prompt_choose_archive(self, parent_widget=None):
        file_path, _ = QFileDialog.getOpenFileName(
            parent_widget,
            "Select Archive Reference",
            "",
            "Archive Files (*.zip *.rar *.7z *.tar *.gz *.bz2 *.xz);;All Files (*)"
        )
        if file_path:
            self.set_archive(file_path)

    prompt_choose_file = prompt_choose_archive
    prompt_choose_asset = prompt_choose_archive
    prompt_choose_pdf = prompt_choose_archive
    prompt_choose_image = prompt_choose_archive
    ...
    def _resolve_abs_path(self) -> Path:
        ...
        rel_or_abs = self.payload.get("archive_path") or self.payload.get("image_path")
        if not rel_or_abs:
            return None
    ...
    def validate_reference(self, force: bool = False) -> bool:
        ...
        if not force and (now - self._last_validated_time) < self.VALIDATION_COOLDOWN_SECONDS:
            return self.state != ArchiveNodeState.MISSING

        self._last_validated_time = now

        archive_path = self.payload.get("archive_path") or self.payload.get("absolute_path") or self.payload.get("image_path")
        if not archive_path:
            if self.state != ArchiveNodeState.EMPTY:
                self.state = ArchiveNodeState.EMPTY
                self.update()
            return False
        ...
        if not exists:
            if self.state != ArchiveNodeState.MISSING:
                self.state = ArchiveNodeState.MISSING
                self.update()
            return False

        if self.state in (ArchiveNodeState.MISSING, ArchiveNodeState.ERROR, ArchiveNodeState.EMPTY):
            try:
                sz = abs_path.stat().st_size
                self.payload["file_size_str"] = self._format_size_bytes(sz)
            except Exception:
    ...
    def paint(self, painter: QPainter, option: QStyleOptionGraphicsItem, widget: QWidget = None):
        ...
        badge_bg = QColor(defn.badge_bg) if defn else QColor("#3F1D2C")
        badge_text_color = QColor(defn.badge_text) if defn else QColor("#F472B6")
        ...
        header_text = f"{icon}  {fname}" if fname else f"{icon}  Archive Reference"
        ...
        if self.state == ArchiveNodeState.EMPTY:
        ...
        elif self.state == ArchiveNodeState.MISSING:
            painter.setFont(QFont("Segoe UI", 9, QFont.Bold))
            painter.setPen(QPen(QColor("#EF4444")))
            filename = self.payload.get("filename") or self.payload.get("file_name") or "Archive"
            painter.drawText(content_rect, Qt.AlignCenter, f"⚠️ Missing Archive\n\n{filename}\nFile no longer exists\n\n[ Locate Archive ]")

        elif self.state == ArchiveNodeState.ERROR:
            painter.setFont(QFont("Segoe UI", 10, QFont.Bold))
            painter.setPen(QPen(QColor("#EF4444")))
            painter.drawText(content_rect, Qt.AlignCenter, "❌\nFailed to Load Archive")

        elif self.state == ArchiveNodeState.READY:
            ...
            painter.setBrush(QBrush(QColor("#351927")))
            painter.setPen(QPen(QColor("#EC4899"), 2))
            painter.drawRoundedRect(doc_rect, 6.0, 6.0)

            ext_upper = (self.payload.get("extension") or "").replace(".", "").upper() or "ZIP"
            painter.setFont(QFont("Segoe UI", 14, QFont.Bold))
            painter.setPen(QPen(QColor("#F472B6")))
            painter.drawText(doc_rect, Qt.AlignCenter, ext_upper)
    ...
    def mouseDoubleClickEvent(self, event):
        """Perform lazy validation on double click. DO NOT launch external applications or extract archives!"""
        self.on_double_clicked(event)
        if event.button() == Qt.LeftButton:
            valid = self.validate_reference(force=True)
            if not valid:
                self.prompt_choose_archive()
            event.accept()
            return
    ...
    def on_context_menu(self, menu):
        ...
        if abs_path and abs_path.exists():
            ...
            rel_path = self.payload.get("archive_path") or self.payload.get("image_path", "")
        ...
        action_relink = QAction("📍 Locate / Relink Archive...", menu)
        action_relink.triggered.connect(lambda: self.prompt_choose_archive())
        menu.addAction(action_relink)

        action_replace = QAction("🔄 Replace Archive...", menu)
        action_replace.triggered.connect(lambda: self.prompt_choose_archive())
        menu.addAction(action_replace)

        if self.payload.get("archive_path") or self.payload.get("absolute_path") or self.payload.get("image_path"):
            action_clear = QAction("❌ Clear Archive Reference", menu)
            action_clear.triggered.connect(lambda: self.set_archive(""))
