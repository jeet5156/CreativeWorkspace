...
class GenericFileNodeState(Enum):
    EMPTY = auto()
    READY = auto()
    MISSING = auto()
    ERROR = auto()
...
class GenericFileNodeItem(NodeItem):
    """Generic File Reference spatial node item for the Lab canvas.

    Renders lightweight document cards displaying unsupported file references, extension,
    ...
    def __init__(self, definition: NodeDefinition, parent=None):
        ...
        self.state = GenericFileNodeState.EMPTY
    ...
    def from_dict(self, data: dict):
        ...
        if "display_mode" not in self.payload:
            self.payload["display_mode"] = "icon"
        if self.payload.get("file_path") or self.payload.get("absolute_path") or self.payload.get("image_path"):
            self.validate_reference(force=True)
        else:
            self.state = GenericFileNodeState.EMPTY
    ...
    def set_file(self, file_path: str, project_location: str = None):
        """Programmatic API to set or replace Generic File reference."""
        self._last_validated_time = 0.0
        ...
        self.payload["format_label"] = f"{ext_upper} File Reference"

        if not abs_path.exists():
            self.payload["file_path"] = str(file_path)
            self.payload["image_path"] = str(file_path)
            self.payload["file_size_str"] = ""
            self.state = GenericFileNodeState.MISSING
            self._emit_modified()
            self.update()
            return
        ...
        self.payload["file_path"] = rel_path
        self.payload["image_path"] = rel_path
        self.state = GenericFileNodeState.READY
        self._emit_modified()
        self.update()
    ...
    def prompt_choose_file(self, parent_widget=None):
    ...
    prompt_choose_asset = prompt_choose_file
    prompt_choose_pdf = prompt_choose_file
    prompt_choose_image = prompt_choose_file
    ...
    def _resolve_abs_path(self) -> Path:
        ...
        rel_or_abs = self.payload.get("file_path") or self.payload.get("image_path")
        if not rel_or_abs:
            return None
    ...
    def validate_reference(self, force: bool = False) -> bool:
        ...
        if not force and (now - self._last_validated_time) < self.VALIDATION_COOLDOWN_SECONDS:
            return self.state != GenericFileNodeState.MISSING

        self._last_validated_time = now

        file_path = self.payload.get("file_path") or self.payload.get("absolute_path") or self.payload.get("image_path")
        if not file_path:
            if self.state != GenericFileNodeState.EMPTY:
                self.state = GenericFileNodeState.EMPTY
                self.update()
            return False
        ...
        if not exists:
            if self.state != GenericFileNodeState.MISSING:
                self.state = GenericFileNodeState.MISSING
                self.update()
            return False

        if self.state in (GenericFileNodeState.MISSING, GenericFileNodeState.ERROR, GenericFileNodeState.EMPTY):
            try:
                sz = abs_path.stat().st_size
                self.payload["file_size_str"] = self._format_size_bytes(sz)
            except Exception:
    ...
    def paint(self, painter: QPainter, option: QStyleOptionGraphicsItem, widget: QWidget = None):
        ...
        badge_bg = QColor(defn.badge_bg) if defn else QColor("#25284A")
        badge_text_color = QColor(defn.badge_text) if defn else QColor("#818CF8")
        ...
        header_text = f"{icon}  {fname}" if fname else f"{icon}  File Reference"
        ...
        if self.state == GenericFileNodeState.EMPTY:
        ...
        elif self.state == GenericFileNodeState.MISSING:
            painter.setFont(QFont("Segoe UI", 9, QFont.Bold))
            painter.setPen(QPen(QColor("#EF4444")))
            filename = self.payload.get("filename") or self.payload.get("file_name") or "File"
            painter.drawText(content_rect, Qt.AlignCenter, f"⚠️ Missing File\n\n{filename}\nFile no longer exists\n\n[ Locate File ]")

        elif self.state == GenericFileNodeState.ERROR:
            painter.setFont(QFont("Segoe UI", 10, QFont.Bold))
            painter.setPen(QPen(QColor("#EF4444")))
            painter.drawText(content_rect, Qt.AlignCenter, "❌\nFailed to Load File")

        elif self.state == GenericFileNodeState.READY:
            ...
            painter.setBrush(QBrush(QColor("#1F2342")))
            painter.setPen(QPen(QColor("#6366F1"), 2))
            painter.drawRoundedRect(doc_rect, 6.0, 6.0)

            ext_upper = (self.payload.get("extension") or "").replace(".", "").upper() or "FILE"
            font_size = 14 if len(ext_upper) <= 3 else (11 if len(ext_upper) <= 5 else 9)
            painter.setFont(QFont("Segoe UI", font_size, QFont.Bold))
            painter.setPen(QPen(QColor("#818CF8")))
    ...
    def mouseDoubleClickEvent(self, event):
        """Perform lazy validation on double click. DO NOT launch external applications!"""
        self.on_double_clicked(event)
        if event.button() == Qt.LeftButton:
            valid = self.validate_reference(force=True)
            if not valid:
                self.prompt_choose_file()
            event.accept()
            return
    ...
    def on_context_menu(self, menu):
        ...
        if abs_path and abs_path.exists():
            ...
            rel_path = self.payload.get("file_path") or self.payload.get("image_path", "")
        ...
        action_relink = QAction("📍 Locate / Relink File...", menu)
        action_relink.triggered.connect(lambda: self.prompt_choose_file())
        menu.addAction(action_relink)

        action_replace = QAction("🔄 Replace File...", menu)
        action_replace.triggered.connect(lambda: self.prompt_choose_file())
        menu.addAction(action_replace)

        if self.payload.get("file_path") or self.payload.get("absolute_path") or self.payload.get("image_path"):
            action_clear = QAction("❌ Clear File Reference", menu)
            action_clear.triggered.connect(lambda: self.set_file(""))
